...
root = Path.cwd()
test_path = root / "tests" / "rentExpiration.integration.test.cjs"

content = r'''// path: tests/rentExpiration.integration.test.cjs
/* eslint-env node */
'use strict';
...
if test_path.exists():
    ...
    if original != content:
        backup = root / ".step5z_backups"
        backup.mkdir(exist_ok=True)
        (backup / "rentExpiration.integration.test.cjs.bak").write_text(original, encoding="utf-8")
