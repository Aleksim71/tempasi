...
describe('seller ownership negative integration contract', () => {
  test('seller-scoped template update affects only templates owned by that seller', async () => {
    const result = await query(`
      ...
      malicious_request(actor_seller_id, requested_template_id, requested_status) AS (
        VALUES
          (101, 202, 'published')
      ),
      unsafe_attempts AS (
        SELECT
          request.actor_seller_id,
          request.requested_template_id,
          templates.seller_id AS real_owner_seller_id,
          request.requested_status
        FROM malicious_request request
        INNER JOIN seller_templates templates
          ON templates.template_id = request.requested_template_id
        WHERE templates.seller_id <> request.actor_seller_id
      ),
      safe_update AS (
        SELECT
        ...
        INNER JOIN seller_templates templates
          ON templates.template_id = request.requested_template_id
         AND templates.seller_id = request.actor_seller_id
      )
      SELECT
        ...
        (SELECT COUNT(*)::int FROM safe_update) AS safely_updated_rows,
        (SELECT COUNT(*)::int FROM unsafe_attempts) AS unsafe_attempts_detected
    `);
