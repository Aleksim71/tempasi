...
# ------------------------------------------------------------
# 1) Fix route placement: /finance/credit-ledger must not be
#    registered inside the /finance handler.
# ------------------------------------------------------------
routes_path = Path("src/web/routes/cabinet.pages.routes.cjs")
...
for variant in nested_variants:
    if variant in routes:
        routes = routes.replace(variant, "")
        removed = True

top_route = "  router.get('/finance/credit-ledger', requireAuthPage, CreditLedgerController.handleCreditLedger);\n\n"
marker = "  router.get('/finance', async (req, res) => {"

if top_route not in routes:
    if marker not in routes:
        raise SystemExit("Could not find /finance route marker")
    routes = routes.replace(marker, top_route + marker)

routes_path.write_text(routes, encoding="utf-8")
print("PATCHED:", routes_path, "removed_nested=", removed)

# ------------------------------------------------------------
# 2) Add real-server smoke test for Finance credit ledger page.
# ------------------------------------------------------------
test_path = Path("tests/financeCreditLedger.page.test.cjs")
test_path.write_text(r'''// path: tests/financeCreditLedger.page.test.cjs
/* eslint-env node */
'use strict';
