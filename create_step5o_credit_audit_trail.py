...
test_path = Path("tests/creditLedger.auditTrail.test.cjs")

content = r'''// tests/creditLedger.auditTrail.test.cjs
...
describe("credit ledger audit trail consistency", () => {
  ...
  it("keeps created and usage rows ordered, unique, and semantically readable", async () => {
    const rows = await listAccountCreditLedger({ db: pool, userId });
