...
test_path = Path("tests/creditLedger.integration.test.cjs")
test_path.write_text(r'''// path: tests/creditLedger.integration.test.cjs
...
async function ensureTestUser(userId) {
  ...
  if (hasColumn(columns, "id")) overrides.id = userId;
...
async function createAccountCredit(userId, amountCents) {
  ...
  if (hasColumn(columns, "id")) overrides.id = id;
...
async function createCreditMovement(tableName, { userId, creditId, movementType, amountCents, createdAt }) {
  const columns = await getColumns(tableName);
  const overrides = {};

  if (hasColumn(columns, "id")) overrides.id = randomUuid();
...
async function closeDb() {
  const closeCandidates = [
    dbModule && dbModule.close,
    dbModule && dbModule.end,
    dbModule && dbModule.pool && dbModule.pool.end,
    dbModule && dbModule.db && dbModule.db.end,
    dbModule && dbModule.default && dbModule.default.end,
    db && db.end,
  ].filter((fn) => typeof fn === "function");

  for (const fn of closeCandidates) {
    try {
      await fn.call(dbModule);
      return;
    } catch (_) {
      // Try next closer.
    }
...
describe("creditLedger real DB integration", () => {
  ...
  test("returns created credit and reserved/applied/released movements from real DB rows", async () => {
    ...
    try {
      ...
      const createdMatches = serialized.match(/created|creation/g) || [];
      expect(createdMatches.length).toBeLessThanOrEqual(2);
    } finally {
